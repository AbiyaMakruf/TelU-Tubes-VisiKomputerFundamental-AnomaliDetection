...
* ![Perbandingan versi YOLO (mAP)](graph_output/yolo%20version%20comparison.png)
* ![Validasi per versi YOLO](graph_output/validation%20-%20yolo%20version%20comparison.png)
* ![Training YOLO11 imgsz 640](graph_output/training%20-%20image%20size%20640.png)
* ![Training YOLO11 imgsz 1024](graph_output/training%20-%20image%20size%201024.png)
* ![Validasi YOLO11 imgsz 640](graph_output/validation%20-%20image%20size%20640.png)
* ![Validasi YOLO11 imgsz 1024](graph_output/validation%20-%20image%20size%201024.png)
* ![Dampak augmentasi pada test set](graph_output/validation%20-%20impact%20augmentation%20to%20test%20set.png)
* ![Confusion Matrix](graph_output/confusion_matrix.png)
* ![Contoh FP/FN](graph_output/FP_FN_1.png)

### Video hasil counting conveyor

* Folder: `video_output/`
* File:

  * `video_conveyor_speed_normal.mp4`
  * `video_conveyor_speed_normal_per_class.mp4`
  * `video_conveyor_speed_up.mp4`

### Model terlatih

* Folder: `results/`
* Contoh model performa tertinggi:
  `results/yolo11_imgsz_1024_augmentation_experiment/training/medium/weights/best.pt`
* Inferensi laporan menggunakan varian **small**, sedangkan counting video menggunakan varian **nano**.

### Prediksi uji

* `results/best_model_predictions/`

### Laporan lengkap

* `laporan/Visi Komputer Fundamental_Tugas Besar_Anomali Detection.pdf`

## 🚀 Cara Pakai Singkat

1. Siapkan environment Python 3 dengan dependensi:
   `ultralytics`, `supervision`, `opencv-python`, `torch`, `roboflow`, `mlflow`, `dagshub`, `matplotlib`, `numpy`, `pandas`.
...
3. Latih/validasi melalui `notebook.ipynb` atau langsung inferensi, contoh:
   ...
   ```bash
   yolo detect predict \
     model="results/yolo11_imgsz_1024_augmentation_experiment/training/medium/weights/best.pt" \
     source="PATH/TO/IMAGES"
   ```
4. Untuk video counting, lihat fungsi `process_video_counting` di notebook dan jalankan dengan sumber `video_conveyor/*.mp4`.
5. Grafik kinerja tersedia di `graph_output/`, video hasil di `video_output/`, dan laporan PDF di `laporan/`.
